<template>
  ...
  onMounted(() => {
    ...
    pegawai.value = pegawaiStore.getPegawaiById(id) as Pegawai | null;
    
    if (pegawai.value) {
      // Look for payroll data
      pegawaiPayroll.value = pegawaiStore.pegawaiPayrollList.find(p => p.pegawaiId === id) as PegawaiPayroll | null;
    } else {
      // Redirect if pegawai not found
